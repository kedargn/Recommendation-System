...
def read_users_age():
	users_age = []
	users_data_file = open("/l/b565/ml-100k/u.user")
	for line in users_data_file:
		users_age.append(int(line.split('|')[1]))                    #array starts from 0. So user id 'x' age will; be stored in users_age[x-1]
	print users_age
	return users_age

def check_age(users_age, main_user, helper_user):
	main_user = main_user-1
	helper_user = helper_user-1
	age_difference = 4
	if(((users_age[main_user]+age_difference)>users_age[helper_user]) and ((users_age[main_user]-age_difference)<users_age[helper_user])):
		print "USER ACCEPTED. Main User %d and potential user %d"%(users_age[main_user], users_age[helper_user])
		return 0
	else:
		print "USER REJECTED. Main User %d and potential user %d"%(users_age[main_user], users_age[helper_user])
		return -1
...
def calculate_age_distance(users_age, user, potential_k_user):
	difference = abs(float(users_age[user]-float(users_age[potential_k_user])))
	return difference**2, difference, difference




def calculate(user_ratings, movies_ratings,user_test_ratings, users_age, movies_test_ratings,euclidean_difference, manhattan_difference, lmax_difference) :
	euclidean_dist = {}
	manhattan_dist = {}
	lmax_dist = {} 
			...
			if(user_ratings.has_key(str(user)) and (not user_ratings[str(user)].has_key(str(movie_id)))):   #'movie' which 'user' hasn't seen
				if(user_test_ratings.has_key(str(user)) and user_test_ratings[str(user)].has_key(str(movie_id)) and movies_ratings.has_key(str(movie_id))): 
					...
					for potential_k_user in potential_k_users:                                                  #users who have seen the "to be predicted" movie
						temp_euclidean = temp_lmax = temp_manhattan = flag = 0
						#if(check_age(users_age, int(user),int(potential_k_user))==0):
						temp_euclidean ,temp_lmax ,temp_manhattan = calculate_age_distance(users_age, int(user)-1, int(potential_k_user)-1)                                 
						for his_movie in user_ratings[str(potential_k_user)].keys():                            #movies seen by potential k users
							if(user_ratings[str(user)].has_key(str(his_movie))):                                    #check if 'user' has seen 'his_movie' seen by potential_k_user
					...
					if(len(euclidean_dist)>0):
						predicted_value_euclidean = avg_predict_rating(user_ratings, euclidean_dist, movie_id, 50)
						predicted_value_manhattan = avg_predict_rating(user_ratings, manhattan_dist, movie_id, 50)
						predicted_value_lmax = avg_predict_rating(user_ratings, lmax_dist, movie_id, 50)
...
def main():
	...
	lmax_difference = []
	users_age = read_users_age()
	files = ["u1.base", "u1.test", "u2.base", "u2.test", "u3.base", "u3.test", "u4.base", "u4.test", "u5.base", "u5.test"]  #change file names
	for i in range(0, len(files), 2):
		user_ratings, movies_ratings = read_user_ratings("/l/b565/ml-100k/"+files[i])
		user_test_ratings, movies_test_ratings = read_user_ratings("/l/b565/ml-100k/"+files[i+1])
		calculate(user_ratings, movies_ratings, user_test_ratings, users_age, movies_test_ratings, euclidean_difference, manhattan_difference, lmax_difference)
		print "In file %s\n"%(files[i])
		#print users list
		#print "**Users**"
		#print user_ratings
	...
	output_file =  open("MAD_5b_age.txt", 'w')
	output_file.write("Age is taken age as another dimension, K=50\n")
