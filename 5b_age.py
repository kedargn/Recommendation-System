			...
			if(user_ratings.has_key(str(user)) and (not user_ratings[str(user)].has_key(str(movie_id)))):   #'movie' which 'user' hasn't seen
				if(user_test_ratings.has_key(str(user)) and user_test_ratings[str(user)].has_key(str(movie_id)) and movies_ratings.has_key(str(movie_id))): 
					...
					if(len(euclidean_dist)>0):
						predicted_value_euclidean = avg_predict_rating(user_ratings, euclidean_dist, movie_id, 15)
						predicted_value_manhattan = avg_predict_rating(user_ratings, manhattan_dist, movie_id, 15)
						predicted_value_lmax = avg_predict_rating(user_ratings, lmax_dist, movie_id, 15)
...
def main():
	...
	output_file.write("Age is taken as another dimension\n")
