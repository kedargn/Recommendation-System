		...
		for movie_id in range(1, int(movie_id_max)+1):                                                    #for each movie in file
			if(user_ratings.has_key(str(user)) and (not user_ratings[str(user)].has_key(str(movie_id)))):   #'movie' which 'user' hasn't seen
				if(user_test_ratings.has_key(str(user)) and user_test_ratings[str(user)].has_key(str(movie_id)) and movies_ratings.has_key(str(movie_id))): 
					...
					predicted_value_euclidean = avg_predict_rating(user_ratings, euclidean_dist, movie_id, 65)
					predicted_value_manhattan = avg_predict_rating(user_ratings, manhattan_dist, movie_id, 65)
					predicted_value_lmax = avg_predict_rating(user_ratings, lmax_dist, movie_id, 65)
...
def main():
	...
	output_file.write("For 10M without any extra dimension, k=65")
