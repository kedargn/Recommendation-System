...
def read_movies(movies_info):
  movies_file = "/l/b565/ml-10M100K/movies.dat"
  movies_file = open(movies_file)
  for movie in movies_file:
  	movie_split = movie.split('::')
  	movies_info[int(movie_split[0])] = movie[2].split('|')

def check_genre(movies_info, common_movie_id, movie):
	for genre in movies_info[movie]:
		for common_movie_genre in movies_info[common_movie_id]:
			if genre ==  common_movie_genre:
				return 0
	return -1
...
def calculate(user_ratings, movies_ratings,user_test_ratings, movies_test_ratings,euclidean_difference, manhattan_difference, lmax_difference, movies_info) :
	euclidean_dist = {}
	manhattan_dist = {}
	lmax_dist = {} 
				...
				if(user_test_ratings.has_key(str(user)) and user_test_ratings[str(user)].has_key(str(movie_id)) and movies_ratings.has_key(str(movie_id))): 
					...
					for potential_k_user in potential_k_users:                                                  #users who have seen the "to be predicted" movie
						...
						for his_movie in user_ratings[str(potential_k_user)].keys():
							if((user_ratings[str(user)].has_key(str(his_movie))) and (check_genre(movies_info, int(his_movie), int(movie_id)))==0):                                    #if 'user' has seen 'his_movie'
					...
					predicted_value_euclidean = avg_predict_rating(user_ratings, euclidean_dist, movie_id, 60)
					predicted_value_manhattan = avg_predict_rating(user_ratings, manhattan_dist, movie_id, 60)
					predicted_value_lmax = avg_predict_rating(user_ratings, lmax_dist, movie_id, 60)
...
def main():
	...
	lmax_difference = []
	movies_info = {}
	read_movies(movies_info)
	files = ["ra.train", "ra.test"]  #change file names
	for i in range(0, len(files), 2):
		...
		calculate(user_ratings, movies_ratings, user_test_ratings, movies_test_ratings, euclidean_difference, manhattan_difference, lmax_difference, movies_info)
		print "In file %s\n"%(files[i])
	...
	output_file =  open("MAD_5c_genre.txt", 'w')
	output_file.write("For 10M without any extra dimension, k=60")
